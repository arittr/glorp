...
- Updated `src/companion/review_capture.rs` to treat `semantic_art_tick_count` as actual consumed semantic ticks instead of counting unique sample buckets, so pre-first-tick paint frames no longer inflate the evidence.
- Added native `render-log.json` evidence fields for `paint_frame_count`, `max_adjacent_base_anchor_delta`, and `max_adjacent_final_anchor_delta`, while keeping `frame_count` for compatibility.
- Added recursive render-log privacy scan coverage that walks serialized string values and rejects leaked source names, token strings, project names, file paths, prompts, responses, raw diagnostics, and pet seed text.
- Removed `.superpowers/sdd/task-4-report.md` from git tracking in a separate cleanup commit so Task 4 source history no longer carries SDD bookkeeping.

### Verification refresh

- `cargo test companion::smooth_timing` -> passed
- `cargo test companion::review_capture::tests::smooth_review_capture -- --nocapture` -> passed
- `cargo test --test cli_smoke companion_ -- --nocapture` -> passed
- `cargo test --test smooth_companion` -> passed
- `cargo fmt --check` -> passed
- `git diff --check` -> passed

### Cleanup commits

- `23323a9` `fix(companion): complete smooth review evidence`
- `cc59673` `chore(sdd): untrack task report`

## Review fix-up: restore pre-existing tracked report

The previous cleanup commit accidentally deleted a pre-existing tracked `.superpowers/sdd/task-4-report.md` from an earlier SDD run. I restored that file exactly from Task 4 base (`70c214d`) in `475a753 chore(sdd): restore prior task report` and moved this run's scratch report to `.superpowers/sdd/task-4-smooth-motion-report.md`, which remains ignored.

Verification:

- `git diff --name-status 70c214d..HEAD -- .superpowers/sdd src/companion` now lists only Task 4 source files under `src/companion` and no `.superpowers/sdd` net changes.

## Smooth startup pet-art fix

### Status

- Fixed. Smooth companion startup now prepares Classic pet art before the first smooth semantic clock tick, while leaving `smooth_semantic_art_tick_index` owned by the existing timer path.

### Commits

- `24e9196` `fix(companion): render smooth pet before first paint`

### Files changed

- `src/companion/app.rs`
- `.superpowers/sdd/task-4-smooth-motion-report.md`

### Failing-test evidence

- `cargo test smooth_startup_prepares_classic_pet_art_before_first_semantic_tick -- --nocapture`
- Expected failure before the fix:

```text
error[E0425]: cannot find function `prepare_smooth_initial_view_model` in this scope
```

### Passing-test evidence

- `cargo test smooth_startup_prepares_classic_pet_art_before_first_semantic_tick -- --nocapture`
- `cargo test companion::app`
- `cargo test companion::smooth_timing`
- `cargo test --test cli_smoke companion_ -- --nocapture`
- `cargo fmt --check`
...
- None. The fix is scoped to smooth startup VM prep and does not change the semantic builder contract in `src/commands/watch.rs`.
